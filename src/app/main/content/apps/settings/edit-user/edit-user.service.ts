...
export class EditUserService implements Resolve<any>
{
    ...
    getUser(userid): Promise<any> {
        return new Promise((resolve, reject) => {
            this.http.get(urlconstants.apiurl + 'users/adminprivilages/' + userid, this.options)
                .subscribe((response: any) => {
                    resolve(response);
                }, reject);
        });
    }
    getUserDataSet(userid): Promise<any> {
        return new Promise((resolve, reject) => {
            this.http.get(urlconstants.apiurl + 'users/dataprivilages/' + userid, this.options)
                .subscribe((response: any) => {
                    resolve(response);
                }, reject);
        });
    }
    UpdateUser(user, dataprivilages): Observable<any> {
        return this.http.post(urlconstants.apiurl + 'users/userUpdate', { data: user, roles: dataprivilages }, this.options)
        // return new Promise((resolve, reject) => {
        //     debugger;
        //     this.http.post(urlconstants.apiurl+'users/userUpdate',{data:user,roles:dataprivilages})
        //         .subscribe((response: any) => {
        //             resolve(response);
        //         }, reject);
        // });
